...
class ValuScanScraper:
    ...
    def scrape_all(self) -> dict[str, Any]:
        ...
        if movement_contract.get("code") == 200:
            ...
            result_data["funds_movement_contract"] = {
        ...
        if movement_spot.get("code") == 200:
            ...
            result_data["funds_movement_spot"] = {
                "count": data.get("total", 0),
                "list": data.get("list", [])
            }
            print(f"   ✓ 获取 {len(data.get('list', []))} 条")
        ...
        gainers_count = data.get("gainers", {}).get("count", 0)
        losers_count = data.get("losers", {}).get("count", 0)
        contract_count = data.get("funds_movement_contract", {}).get("count", 0)
        spot_count = data.get("funds_movement_spot", {}).get("count", 0)
        ai_count = data.get("ai_messages", {}).get("total", 0)
        warn_count = data.get("warn_messages", {}).get("count", 0)
        print(f"  - ????????? {gainers_count} ???")
        print(f"  - ????????? {losers_count} ???")
        print(f"  - ????????????(??????): {contract_count} ???")
        print(f"  - ????????????(??????): {spot_count} ???")
        print(f"  - AI??????: {ai_count} ???")
        print(f"  - ????????????: {warn_count} ???")
        print("=" * 60)
