#!/usr/bin/env python3
# ruff: noqa: RUF002, E501, C901, S110
"""
...
class ValuScanClient:
    ...
    def __init__(self, token_file: Path | None = None, proxy: str | None = None):
        ...
        self._access_ticket_cache: str | None = None

    @staticmethod
    def _format_coin_key(symbol: str | None, chain: str | None) -> str:
        if not symbol:
            return ""
    ...
    @staticmethod
    def _is_all_time(value: str | None) -> bool:
        if value is None:
            return False
        key = str(value).strip().lower()
        return key in ("all", "*", "alltime", "all_time", "all-times", "all_time_windows")

    @staticmethod
    def _parse_time_list(items: Any) -> list[str]:
        times: list[str] = []
        if isinstance(items, list):
            for item in items:
                if isinstance(item, str) and item.strip():
                    times.append(item.strip())
                    continue
                if isinstance(item, dict):
                    for key in ("time", "timeParticleEnum", "timeParticle", "period", "value"):
                        value = item.get(key)
    ...
    def _get_time_list(self, endpoint: str) -> list[str]:
        payload = self._request("GET", endpoint)
        if not isinstance(payload, dict):
            return []
        return self._parse_time_list(payload.get("data"))
    ...
    def _load_access_ticket(self) -> str | None:
    ...
    def get_funds_movement(
        ...
        """获取资金异动列表"""
        payload: dict[str, Any] = {"pageNum": page, "pageSize": page_size}
        if chain:
            payload["chain"] = chain
    ...
    def get_funds_update(self, last_id: int | None = None) -> dict[str, Any]:
        """获取资金异动更新"""
        params = {}
        if last_id:
    ...
    def get_coin_rank(
        ...
        """获取涨跌榜?"""
        order_col = order_by or "percentChange24h"
        if order_by is None:
            if time_range == "1h":
    ...
    def get_gainers(self, page: int = 1, page_size: int = 20, time_range: str = "24h") -> dict[str, Any]:
        """获取涨幅榜"""
        return self.get_coin_rank(1, page, page_size, time_range=time_range)

    def get_losers(self, page: int = 1, page_size: int = 20, time_range: str = "24h") -> dict[str, Any]:
        """获取跌幅榜"""
        return self.get_coin_rank(2, page, page_size, time_range=time_range)

    # ==================== 代币信息 ====================

    def get_token_detail(self, symbol: str, chain: str | None = None) -> dict[str, Any]:
        """获取代币详情"""
    ...
    def get_holder_analysis(self, symbol: str, chain: str | None = None) -> dict[str, Any]:
        """获取持仓分析"""
        payload = {"symbol": symbol}
        if chain:
            payload["chain"] = chain
    ...
    @staticmethod
    def _merge_trade_type_payloads(
        spot_payload: dict[str, Any],
        futures_payload: dict[str, Any],
    ) -> dict[str, Any]:
        spot_ok = isinstance(spot_payload, dict) and spot_payload.get("code") == 200
        futures_ok = isinstance(futures_payload, dict) and futures_payload.get("code") == 200
        result = {
            "code": 200 if spot_ok and futures_ok else 500,
            "msg": "success" if spot_ok and futures_ok else "partial",
            "data": {
                "spot": spot_payload.get("data") if spot_ok else None,
                "futures": futures_payload.get("data") if futures_ok else None,
            },
        }
        if not spot_ok or not futures_ok:
            errors: dict[str, Any] = {}
            if not spot_ok:
                errors["spot"] = spot_payload
            if not futures_ok:
                errors["futures"] = futures_payload
            result["errors"] = errors
        return result

    def get_heat_map(self, interval: str = "1h") -> dict[str, Any]:
        """Get liquidation heat map data by interval."""
        return self._request(
            "GET",
            "/api/account/message/getHeatMap",
    ...
    def get_fund_trade_history_total(
        ...
        spot_payload = self._request("POST", "/api/trade/fundTradeHistoryTotal", json_body={
            "timeParticle": time_particle,
            "limitSize": limit_size,
            "flow": bool(flow),
            "keyword": keyword,
        ...
        futures_payload = self._request("POST", "/api/trade/fundTradeHistoryTotal", json_body={
            "timeParticle": time_particle,
            "limitSize": limit_size,
            "flow": bool(flow),
            "keyword": keyword,
        ...
        return self._merge_trade_type_payloads(spot_payload, futures_payload)
    ...
    def get_token_flow(self, time_period: str = "H12", page: int = 1, page_size: int = 20,
        ...
        Args:
            time_period: 时间周期 (H1/H4/H8/H12/D1/D2/D3/D7/D10/D15/D30/D60/D90/D120/D150/D180, 传 all 获取全
                部时间窗)
            page: 页码
            page_size: 每页数量
        ...
        """
        if time_period is None or self._is_all_time(time_period):
            times = self._get_time_list("/api/analysis/coin/getCoinExchangeFlowPageTime")
            if not times:
                times = ["H12"]
            data: dict[str, Any] = {}
            errors: dict[str, Any] = {}
            for period in times:
                resp = self._request("POST", "/api/analysis/coin/getCoinExchangeFlowPage", json_body={
                    "page": page,
                    "pageSize": page_size,
                    "order": [{"column": order_by, "asc": asc}],
                    "filters": [],
    ...
    def get_whale_flow(self, trade_type: int | None = None, time_period: str = "m5", page: int = 1,
        ...
        """
        def _fetch(period: str, trade_type_value: int) -> dict[str, Any]:
            return self._request("POST", "/api/trade/getTimeTradePage", json_body={
                ...
                "filters": [],
                "time": period,
                "type": trade_type_value,
            })

        if time_period is None or self._is_all_time(time_period):
            times = self._get_time_list("/api/trade/getTradePageTime")
            if not times:
                times = ["m5"]
            data: dict[str, Any] = {}
            errors: dict[str, Any] = {}
            for period in times:
                if trade_type in (1, 2):
                    ...
                    spot_payload = _fetch(period, 1)
                    futures_payload = _fetch(period, 2)
                    merged = self._merge_trade_type_payloads(spot_payload, futures_payload)
                    data[period] = merged.get("data")
                    if merged.get("code") != 200:
                        errors[period] = merged.get("errors", merged)
            result = {"code": 200 if not errors else 500, "msg": "success" if not errors else "partial", "data": data}
            if errors:
                result["errors"] = errors
            return result
        ...
        if trade_type in (1, 2):
            return _fetch(time_period, trade_type)
        spot_payload = _fetch(time_period, 1)
        futures_payload = _fetch(time_period, 2)
        return self._merge_trade_type_payloads(spot_payload, futures_payload)
...
# 便捷函数
_default_client: ValuScanClient | None = None
