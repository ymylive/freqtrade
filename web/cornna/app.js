...
const translations = {
  ...
  hero_eyebrow: { en: "CCXT Market Intelligence / AI-Tuned Execution", zh: "CCXT 行情智能 / AI 微调执行" },
  hero_title_a: { en: "Quant signals shaped by live flow,", zh: "量化信号由实时资金流塑形，" },
  hero_title_b: { en: "calibrated in real time.", zh: "并实时校准。" },
  hero_lead: {
    en: "Cornna distills CCXT market data, momentum, and volatility into a compact decision layer. No clutter, only signals that matter.",
    zh: "Cornna 将 CCXT 行情、动量与波动收敛为精简决策层。去除噪声，只保留关键信号。",
  },
  cta_console: { en: "Open Console", zh: "打开控制台" },
  metric_flow_bias: { en: "Trend Bias", zh: "趋势倾向" },
  metric_flow_hint: { en: "EMA spread", zh: "均线差距" },
  metric_whale_pressure: { en: "Momentum", zh: "动量" },
  metric_whale_hint: { en: "1h change", zh: "1 小时涨跌" },
  metric_sentiment: { en: "Volatility", zh: "波动率" },
  metric_sentiment_hint: { en: "ATR percent", zh: "ATR 百分比" },
  panel_signal_title: { en: "Signal Summary", zh: "信号摘要" },
  panel_signal_body: {
    en: "Compact scoring across trend, momentum, and volatility. Thresholds self-adjust to trading outcomes.",
    zh: "趋势、动量与波动率的紧凑评分。阈值随交易结果自动校准。",
  },
  pill_flow: { en: "Trend", zh: "趋势" },
  pill_sentiment: { en: "Momentum", zh: "动量" },
  pill_volatility: { en: "Volatility", zh: "波动率" },
  pill_risk: { en: "Risk", zh: "风险" },
  panel_exec_title: { en: "Execution Logic", zh: "执行逻辑" },
  panel_exec_body: {
    en: "Simple decision tree. Real-time CCXT data only. No historic backtest assumptions.",
    zh: "简洁决策树，仅使用实时 CCXT 行情数据，不依赖历史回测假设。",
  },
  exec_entry: { en: "Entry", zh: "入场" },
  exec_entry_desc: { en: "Trend + momentum alignment", zh: "趋势与动量共振" },
  exec_exit: { en: "Exit", zh: "离场" },
  exec_exit_desc: { en: "Momentum fade + trend flip", zh: "动量衰减 + 趋势反转" },
  ...
  pipeline_ingest_body: { en: "CCXT OHLCV, order flow, volatility cues.", zh: "CCXT 行情、订单流与波动线索。" },
  ...
  monitor_alert_1: { en: "Trend shift detected", zh: "检测到趋势切换" },
  monitor_alert_2: { en: "Momentum threshold crossed", zh: "动量突破阈值" },
  monitor_alert_3: { en: "Volatility spike flagged", zh: "波动率出现尖峰" },
  ...
  insight_body: {
    en: "A single view of price momentum, volatility drift, and order flow. Keep the surface area minimal, stay in control.",
    zh: "单页呈现价格动量、波动变化与订单流。界面极简，掌控全局。",
  },
  cta_snapshot: { en: "Download Snapshot", zh: "下载快照" },
  netflow_btc: { en: "1h Change +4.1%", zh: "1 小时 +4.1%" },
  netflow_eth: { en: "1h Change +3.3%", zh: "1 小时 +3.3%" },
  netflow_sol: { en: "1h Change -1.2%", zh: "1 小时 -1.2%" },
  netflow_arb: { en: "1h Change +2.6%", zh: "1 小时 +2.6%" },
  footer_title: { en: "Cornna Quant Console", zh: "Cornna 量化控制台" },
  footer_body: { en: "Signal clarity for live CCXT market intelligence.", zh: "为实时 CCXT 行情提供清晰信号。" },
...
const metricNodes = {
  trend_bias: document.querySelector('[data-metric="trend_bias"]'),
  momentum: document.querySelector('[data-metric="momentum"]'),
  volatility: document.querySelector('[data-metric="volatility"]'),
...
const applyMonitorData = (payload) => {
  ...
  if (metricNodes.trend_bias) metricNodes.trend_bias.textContent = formatSigned(payload.trend_bias);
  if (metricNodes.momentum) metricNodes.momentum.textContent = formatPercent(payload.momentum);
  if (metricNodes.volatility) metricNodes.volatility.textContent = formatPercent(payload.volatility);
